...
async def get_metadata(network: Optional[str] = None,
    ...
    results = validator.miner_discovery_manager.get_miners_by_network(network)
    return results
...
async def get_receipts(miner_key: str, page: int = 1, page_size: int = 10,
                       validator: Validator = Depends(get_validator),
                       api_key: str = Depends(api_key_auth)):
    results = validator.miner_receipt_manager.get_receipts_by_miner_key(miner_key, page, page_size)
    return results


@miner_router.get("/miner/multiplier")
async def get_receipt_multiplier(miner_key: Optional[str], network: Optional[str] = None,
                                       validator: Validator = Depends(get_validator),
                                       api_key: str = Depends(api_key_auth)):
    results = validator.miner_receipt_manager.get_receipt_miner_multiplier(network, miner_key)
    return results
...
async def get_ranks(network: Optional[str] = None,
                          validator: Validator = Depends(get_validator),
                          api_key: str = Depends(api_key_auth)):
    results = validator.miner_discovery_manager.get_miners_for_leader_board(network)
    return results