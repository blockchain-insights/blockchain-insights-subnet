import os
from src.subnet.protocol.llm_engine import Query, LLM_ERROR_SEARCH_TARGET_NOT_SUPPORTED, \
    LLM_ERROR_SEARCH_LIMIT_NOT_SPECIFIED, LLM_ERROR_SEARCH_LIMIT_EXCEEDED, LLM_ERROR_TYPE_NOT_SUPPORTED, \
    QUERY_TYPE_SEARCH
...
class QueryBuilder:
    ...
    class __InnerQueryBuilder:
        ...
        def build_search_query(query: Query) -> str:
            if query.target is None:
                raise Exception(LLM_ERROR_SEARCH_TARGET_NOT_SUPPORTED)
            if query.limit is None:
                raise Exception(LLM_ERROR_SEARCH_LIMIT_NOT_SPECIFIED)
            if query.limit > os.getenv('QUERY_MAX_LIMIT', 50):
                raise Exception(LLM_ERROR_SEARCH_LIMIT_EXCEEDED)
    ...
    def build_query(query: Query) -> str:
        if query.type is None:
            raise Exception(LLM_ERROR_TYPE_NOT_SUPPORTED)
        if query.type == QUERY_TYPE_SEARCH:
